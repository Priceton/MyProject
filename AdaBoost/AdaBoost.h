...
int AdaBoostTrain(float* X, short* Y, int NSample, int NFeature, int iterN, char* fileName){
    ...
    float *W= new float[NSample], tw=1.0/NSample, R;
    float * CH = new float[NSample], *F = new float[NSample];
    float * H = new float[NSample];
    node * tX=new node[NSample];
    float weightedError, alpha, totalW;
    float trueError;
    int i,j,CC=0;
    int LP=0, LN=0;
    for (i=0;i<NSample;i++){
    ...
    typedef std::pair<float, int> sort_index_pair;
    ...
    while (CC<iterN){
        ...
        for (i=0;i<NSample;i++){
            if (bwl.sign==1)
                CH[i] = 2*float(bwl_xpos[i]>bwl.threshold) - 1;
            else
                CH[i] = 2*float(bwl_xpos[i]<=bwl.threshold) - 1;
            F[i] = float(CH[i]==Y[i]);
            H[i] += alpha*CH[i];
...
// X: input testing samples. An 1D array of size NSamplexNFeature,
...
void AdaBoostClassify(float *X, int NSample, short NFeature, int LC, short *featureID, float *alpha, short *sign, float *threshold, float* H, float* cH){
    int i,j;
